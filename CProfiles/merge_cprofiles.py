...
def main():
    ...
    # loop through each momentum group
    nphotons = [] # mean nphotons per momentum
    sthrs    = [] # s value at which 90% of photons are emitted per momentum
    for momentum, files in zip(momenta, groups):

        if args.verbose: print(f"Processing {momentum} MeV/c files...".ljust(50))
        ...
        # save mean nphotons for this momentum
    ...
    # save s thresholds
    g = ROOT.TGraph(len(momenta), momenta, np.array(sthrs))
