...
ClassImp(TScatTableF);

//double interpvals[3][3][3][3][3][3];

TScatTableF::TScatTableF(){

}

TScatTableF::TScatTableF(TScatTableF* tst){
...
TScatTableF::TScatTableF(TScatTable* tst){
...
TScatTableF::TScatTableF(const char *name, const char *title, int nzsbins, double minzs, double maxzs, int nrsbins, double minrs, double maxrs, int ntbins, double mint, double maxt, int nastbins, double minast, double maxast, int nctbins, double minct, double maxct, int nphibins, double minphi, double maxphi){
...
bool TScatTableF::Initialize(const char *name, const char *title, int nzsbins, double minzs, double maxzs, int nrsbins, double minrs, double maxrs, int ntbins, double mint, double maxt, int nastbins, double minast, double maxast, int nctbins, double minct, double maxct, int nphibins, double minphi, double maxphi){
  ...
  if (nzsbins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: nzsbins = " << nzsbins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (nrsbins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: nrsbins = " << nrsbins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (ntbins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: ntbins = " << ntbins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (nastbins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: nastbins = " << nastbins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (nctbins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: nctbins = " << nctbins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (nphibins<=0) {
    std::cerr << "ERROR in TScatTableF::Initialize: nphibins = " << nphibins << ", which is <= 0" << std::endl;
    badentries = true;
  }
  if (badentries) {
    return badentries;
...
void TScatTableF::fillbininfo(){
  ...
  for (int idim=0; idim<maxdimensions; idim++) {    
    ...
    bnwdthrecp[idim]=1./bnwdth[idim];
  }
  
}

void TScatTableF::Reset(){
  for (int iele=0; iele<=nbinstot; iele++) {
    table[iele] = 0.;
  }  
}

int TScatTableF::GetIndex(int* subindices){

  int index=0;
  for (int idim=maxdimensions-1; idim>=0; idim--) {
    if (subindices[idim] >= nbins[idim]) {
      std::cerr << "ERROR in TScatTableF::GetIndex: subindex " << idim << " (" << subindices[idim] << ") is out of bounds (range = [0," << nbins[idim]-1 << "])" << std::endl;
      char a[10];
      std::cin >> a;
      return -1;
    }
...
int TScatTableF::GetIndexFast(int* subindices){  
  int index=0;
  for (int idim=maxdimensions-1; idim>=0; idim--) {
    index *= nbins[idim];
    index += subindices[idim];
...
int TScatTableF::GetIndex(int izs, int irs, int it, int iast, int ict, int iphi){
...
void TScatTableF::GetSubIndices(int* subindices, int iglobalindex){
...
bool TScatTableF::IsInABin(double zs, double rs, double t, double ast, double ct, double phi){
...
double TScatTableF::GetBinCenter(int idimension, int ibin) {
...
void TScatTableF::FindClosestSubBins(int* ibinarray, double zs, double rs, double t, double ast, double ct, double phi){
...
void TScatTableF::FindSurroundingBins(int& min, int& max, int idim, double val){

  if (idim >= maxdimensions) {
    std::cerr << "ERROR in TScatTableF::FindSurroundingBins: idim (=" << idim << ") > maxdimensions (=" << maxdimensions << ")" << std::endl;
    min = 0;
    max = 0;
    return;
  }
...
void TScatTableF::SetElement(int iglobalindex, double val){

  table[iglobalindex] = val;

}

void TScatTableF::SetElement(int izs, int irs, int it, int iast, int ict, int iphi, double val){
  ...
  if (table.size() > 0) {
    ...
    std::cerr << "ERROR in TScatTableF::SetElement: table has not yet been initialized" << std::endl;
  }

}

void TScatTableF::SetElement(int izs, int irs, int it, int iast, double val){

  SetElement(izs, irs, it, iast, 0, 0, val);

}

double TScatTableF::GetElement(int iglobalindex){

  return table[iglobalindex];

}

double TScatTableF::GetElement(int izs, int irs, int it, int iast, int ict, int iphi){
...
double TScatTableF::GetElement(int* subindices){
  ...
  if (table.size() > 0) {
    ...
    std::cerr << "ERROR in TScatTableF::GetElement: table has not yet been initialized" << std::endl;
  }
...
double TScatTableF::GetElement6DFast(int izs, int irs, int it, int iast, int ict, int iphi){
...
double TScatTableF::GetElement6DFast(int* subindices){
...
double TScatTableF::GetInterpVal(double zs, double rs, double t, double ast, double ct, double phi){
...
//  }
  ...
  for (int idim=0; idim<maxdimensions; idim++) {
    ...
    if (interpfunc < 0.) {
      std::cout << "ERROR in TScatTableF::GetInterpVal(). interpfunc = " << interpfunc << std::endl;
...
double TScatTableF::GetInterpVal2(double zs, double rs, double t, double ast, double ct, double phi){
...
double TScatTableF::GetInterpVal6DFast(double zs, double rs, double t, double ast, double ct, double phi){
  ...
  for (int i=0; i<6; i++) {
    double dtmp=(vals[i]-binCtr[i][0])*bnwdthrecp[i];
    binarr[i]=(int)dtmp;//index of closest bin center smaller than val[i]
    if (binarr[i]<0) {
      binarr[i]=0;
      binincrar[i]=0;
      wt[i]=0.;
...
double TScatTableF::Interp(double x, double x1, double x2, double y1, double y2){
...
void TScatTableF::Fill(double zs, double rs, double t, double ast, double ct, double phi, double wgt){
...
void TScatTableF::Fill(double zs, double rs, double t, double ast, double wgt){

  Fill(zs, rs, t, ast, 0, 0, wgt);

}

TH1F* TScatTableF::MakeTH1F(const char *name, const char *title, int subindex){

  if ((subindex >= maxdimensions)||(subindex < 0)) {
    std::cerr << "ERROR in TScatTableF::MakeTH1F: subindex (" << subindex << ") is out of bounds" << std::endl;
    return NULL;
  }
...
TH2F* TScatTableF::MakeTH2F(const char *name, const char *title, int subindex1, int subindex2){

  if ((subindex1 >= maxdimensions)||(subindex1 < 0)) {
    std::cerr << "ERROR in TScatTableF::MakeTH2F: subindex1 (" << subindex1 << ") is out of bounds" << std::endl;
    return NULL;
  }
  if ((subindex2 >= maxdimensions)||(subindex2 < 0)) {
    std::cerr << "ERROR in TScatTableF::MakeTH2F: subindex2 (" << subindex2 << ") is out of bounds" << std::endl;
    return NULL;
  }
...
TScatTableF* TScatTableF::Make4DScatTable(){
  ...
  TScatTableF* tst = new TScatTableF(tname,ttitle,
...
int TScatTableF::GetNBinsTot(){

  return nbinstot;

}

int TScatTableF::GetNBins(int idim){

  return nbins[idim];

}

double TScatTableF::GetAxisBound(int idim, int ibound){

  return axisbounds[idim][ibound];

}

double TScatTableF::GetIntegral(){
  double sum = 0.;
...
void TScatTableF::Add(TScatTableF* tst, double c1){
  
  if (nbinstot != tst->GetNBinsTot()) {
    std::cerr << "ERROR in TScatTableF::Add. Both scattering tables must be the same length" << std::endl;
    return;
  }
...
void TScatTableF::Subtract(TScatTableF* tst, int cutValue){

  if (nbinstot != tst->GetNBinsTot()) {
    std::cerr << "ERROR in TScatTableF::Subtract. Both scattering tables must be the same length" << std::endl;
    return;
  }
  ...
  for (int ibin=0; ibin<nbinstot; ibin++) {
    ...
    if (table[ibin] < tst->GetElement(ibin)) {
      //      std::cout << "Strange: In TScatTableF::Subtract (a - b), a = " << table[ibin] << " and b = " << tst->GetElement(ibin) << std::endl;
      nmoredirect++;
      table[ibin] = 0.;
      continue;
    }
  ...
  std::cout << "In TScatTableF::Subtract, found more direct light than scattered light " << nmoredirect << " times." << std::endl;

}

void TScatTableF::Divide4D(TScatTableF* tst){
...
void TScatTableF::DivideUnnormalized4D(TScatTableF* tst){
...
template < typename T > void TScatTableF::PrintVector(std::vector<T> vec){
...
void TScatTableF::PrintMinElement(){
...
TH1F* TScatTableF::MakeNPhotHisto(const char* name, const char* title, int nxbins, double xlow, double xhigh) {
...
void TScatTableF::CheckDiff(TScatTableF* tst){
