...
void check ( std::string inList, std::string outFName, int isNuPRISM = 0){
  ...
  if(isNuPRISM){
    //    double detRadius = 400.;
    //    double detLength = 500.*2.;
    double detRadius = 371.;
    double detLength = 1042.;
  }
  ...
  cout << "Read PMT position" << endl;
  int tubeNo;
  int NumPMT = geoWCSim->GetWCNumPMT();
  for (int i = 0; i < NumPMT; i++){
    tubeNo  = geoWCSim->GetPMT(i).GetTubeNo();
    posMap[tubeNo] = TVector3(geoWCSim->GetPMT(i).GetPosition(0),
		              geoWCSim->GetPMT(i).GetPosition(1),
	                      geoWCSim->GetPMT(i).GetPosition(2));

    dirMap[tubeNo] = TVector3(geoWCSim->GetPMT(i).GetOrientation(0),
  ...
  cout << "Prepare histograms" << endl;
  TFile * outRootFile = new TFile(outFName.c_str(), "RECREATE");
  
  std::vector<TH1F*> hPMTR;
  std::vector<TH1F*> hPMTZ;
  ...
  for (int j =0; j < 3; j++){
    sprintf(nbuff, "PMTPosR_%d", j);
    sprintf(buff, "R [cm]");
    temp = new TH1F(nbuff, buff, 400, 0, 400);
    hPMTR.push_back(temp);
    hPMTR.back()->Sumw2();

    sprintf(nbuff, "PMTPosZ_%d", j);
    sprintf(buff, "Z [cm]");
    temp = new TH1F(nbuff, buff, 1100, -550, 550);
    hPMTZ.push_back(temp);
    hPMTZ.back()->Sumw2();
  }
  ...
  TVector3 thisPMTpos;
  for (tubeNo = 1; tubeNo <= NumPMT; tubeNo++){
    thisPMTpos = TfmDetToGlblCoord(posMap[tubeNo], true,  true);
    if(tubeNo%19==0){
      hPMTR.at(0)->Fill(thisPMTpos.Perp());
      hPMTZ.at(0)->Fill(thisPMTpos.Z());
    }else if(tubeNo%19<13){
      hPMTR.at(2)->Fill(thisPMTpos.Perp());
      hPMTZ.at(2)->Fill(thisPMTpos.Z());
    }else{
      hPMTR.at(1)->Fill(thisPMTpos.Perp());
      hPMTZ.at(1)->Fill(thisPMTpos.Z());
    }
  }
...
//    r.push_back(50);
  ...
  TH1F *hR = new TH1F("PhotonPosR", ";R [cm]", 500, 0., 500.);
  TH1F *hZ = new TH1F("PhotonPosZ", ";z [cm]", 1200, -600., 600.);
  ...
  for (int i =0; i < r.size(); i++){
    ...
    //    sprintf(buff, "Angular response function at %i cm (only barrel PMTs)", (int) r.at(i).);
      ...
      sprintf(nbuff, "angRespEndCap_%i_%d", (int) r.at(i), j);
  ...
  //  for (int evt = 0; evt < 10 ; evt++){
    ...
    if(isct_ != 0) continue; // Only direct light 

    if(!(evt%10000)) cout << std::rootFileNames.at(0) << "\t" << evt << std::endl;
    ...
    hR->Fill(thisTrackPos.Perp());
    hZ->Fill(thisTrackPos.Z());
	...
	if (inFiducialZ || inFiducialR){ // Make sure spherical shell is fully contained within "fiducial" volume 
	  ...
	  if(tubeNo%19==0){
            hAllPe.at(shell*3)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	  }else if(tubeNo%19<13){
            hAllPe.at(shell*3+2)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	  }else{
            hAllPe.at(shell*3+1)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	  }

	  if (inFiducialZ) { // Must be a barrel PMT
            if(tubeNo%19==0){
	      hBarrelPe.at(shell*3)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
            }else if(tubeNo%19<13){
	      hBarrelPe.at(shell*3+2)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	    }else{
      	      hBarrelPe.at(shell*3+1)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	    }
	    
	  } else { // It's an endcap PMT
            if(tubeNo%19==0){
              hEndCapPe.at(shell*3)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
            }else if(tubeNo%19<13){
	      hEndCapPe.at(shell*3+2)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	    }else{
      	      hEndCapPe.at(shell*3+1)->Fill(relPos.Dot(thisPMTdir)/relPos.Mag(), totalPe);
	    }
  ...
  for (int i = 0 ; i < r.size(); i++){
    for (int j = 0 ; j < 3; j++){
      ...
      if (hAllPe.at(i*3+j)->Integral()) hAllPe.at(i*3+j)->      Scale(1/hAllPe.at(i*3+j)->   GetBinContent(nBins));
      if (hBarrelPe.at(i*3+j)->Integral()) hBarrelPe.at(i*3+j)->Scale(1/hBarrelPe.at(i*3+j)->GetBinContent(nBins)); 
      if (hEndCapPe.at(i*3+j)->Integral()) hEndCapPe.at(i*3+j)->Scale(1/hEndCapPe.at(i*3+j)->GetBinContent(nBins));
    }
  }
